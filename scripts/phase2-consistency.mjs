/**
 * Phase 2: segment cluster normalization + consistency checks (same pattern as phase 3).
 * Run: node scripts/phase2-consistency.mjs
 *
...
const PHASE_FILE = 'data/phase2.json'
const REPORT_FILE = 'scripts/phase2-consistency-report.json'
...
function run() {
  ...
  console.log(JSON.stringify({ phase: 2, before, after, reportIssues: issues.length }, null, 2))
  console.error(
    `\nPhase 2 consistency — BEFORE: join=${before.join} missing=${before.missing} sortPair=${before.sortPair} alien=${before.alien}`,
  )
  console.error(
    `Phase 2 consistency — AFTER:  join=${after.join} missing=${after.missing} sortPair=${after.sortPair} alien=${after.alien}`,
  )
